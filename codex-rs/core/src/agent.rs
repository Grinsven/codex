...
impl AgentRegistry {
    ...
    fn validate_prompt_path(base_dir: &Path, prompt_file: &str) -> anyhow::Result<PathBuf> {
        ...
        let canonical_base = base_dir
            .canonicalize()
            .map_err(|e| anyhow::anyhow!("Cannot access prompts directory: {e}"))?;

        // Canonicalize to resolve ../ and symlinks. If the path traversal makes
        // the target unreachable (e.g., because the intermediate path does not
        // exist), treat it as a security violation instead of a missing file.
        let canonical = match path.canonicalize() {
            ...
            Err(err) => {
                ...
                return Err(anyhow::anyhow!("Cannot access prompt file: {err}"));
            }
        };
