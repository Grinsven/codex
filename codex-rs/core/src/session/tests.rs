...
async fn build_initial_context_emits_thread_start_skill_warning_on_repeated_builds() {
    ...
    assert!(matches!(
        ...
        EventMsg::Warning(WarningEvent { message })
            if message == "Warning: Exceeded skills context budget of 5%. All skill descriptions were removed and 2 additional skills were not included in the model-visible skills list."
    ));
...
#[tokio::test]
async fn build_initial_context_respects_disabled_skill_budget_warnings() {
    let (session, turn_context, rx) = make_session_and_context_with_rx().await;
    let mut turn_context = Arc::into_inner(turn_context).expect("sole turn context owner");
    let mut outcome = SkillLoadOutcome::default();
    outcome.skills = vec![
    ...
    Arc::make_mut(&mut turn_context.config).show_skill_context_budget_warnings = false;
    turn_context.model_info.context_window = Some(100);
    turn_context.turn_skills = TurnSkillsContext::new(Arc::new(outcome));
